...
def llegadas(): return np.random.exponential(3/ 1)  # Random para llegadas de clientes (1 cliente por minuto)


def aleatorio(caje): return np.random.exponential(
...
def cliente(env, num, cajeros, comida, num_comida, caf):
    with cajeros.request() as solicitud:
        ...
        if caf.disponible[comida] < num_comida:
            ...
            print(
                f"El cliente {num} se fue del establecimiento porque no hay las unidades de {comida} que quería o el producto se agotó \n",
                f"Tiempo Salida: {env.now}", '\n <-------------------------------------------------------------->')
            return
        ...
        if caf.disponible[comida] == 0:
            ...
            print(f"LAS UNIDADES DE {comida} SE AGOTARON A LOS {caf.momento_sin_op[comida]} MINUTOS")
...
NUM_ITEMS = 25# Número de items por opción
opciones = [
    'Empanada y Jugo',
    'Galleta y Avena',
    'Café y Croissant',
...
disponible = {op: NUM_ITEMS for op in opciones}  # Cantidad existente de cada producto
sin_opcion = {op: env.event() for op in opciones}  # Eventos en los que cada opción se agota
momento_sin_op = {op: None for op in opciones}  # Tiempo en el que la opción agotó sus existencias
...
cajeros = sim.Resource(env, capacity=caje)  # Indica que la capacidad de atención de la cafetería se limita al número de cajeros
env.run(tiempo)  # Ejecutar entorno

print(momento_sin_op)  # Producto con menor tiempo registrado, producto más vendido
